...
### Userbar items refactoring

The items in the Wagtail userbar have been refactored to [Template components](template_components). This changes the internal API for custom userbar items.

* The undocumented `BaseItem` and its subclasses now inherit from `Component`.
* The undocumented `template` attribute on userbar items has been renamed to `template_name` to align with the `Component` API.
* The undocumented `get_context_data` method now receives a `parent_context` parameter, instead of a `request` parameter.
* The documented `render(request)` method has been replaced with `render_html(parent_context)`. For backwards compatibility, the `construct_wagtail_userbar` hook still accepts objects with a `render(request)` method.
