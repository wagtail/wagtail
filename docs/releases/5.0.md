...
Additionally, the Python `StreamBlockValidationError`, `ListBlockValidationError` and `StructBlockValidationError` classes no longer provide a `params` dict with `block_errors` and `non_block_errors` items; these are now available as the attributes `block_errors` and `non_block_errors` on the exception itself.
