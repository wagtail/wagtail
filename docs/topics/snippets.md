...
To customise the base queryset for the listing view, you could override the {meth}`~wagtail.snippets.views.snippets.SnippetViewSet.get_queryset` method. Additionally, the {attr}`~wagtail.snippets.views.snippets.SnippetViewSet.ordering` attribute or the {meth}`~wagtail.snippets.views.snippets.SnippetViewSet.get_ordering` method can be used to specify the default ordering of the listing view.
