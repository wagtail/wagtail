...
-   **Unlock** - grants any other user (with the right permission) the ability to edit this page. It can be used to remove the lock on a page, allowing the user who locked the page, or any other authorized users, to edit it. This permission ensures flexibility for scenarios where exclusive editing access is required or when the lock needs to be overridden for collaboration.
