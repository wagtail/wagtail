...
class ModelViewSet(ViewSet):
    ...
    def url_converter(self):
        """What will be used to convert pks in urls"""
    ...
    def on_register(self):
        super().on_register()
        if self.url_converter not in get_converters():
            raise ImproperlyConfigured(
                f"{self.__class__.__name__} is using an unknown URL converter"
            )
