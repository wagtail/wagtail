...
function InlinePanel(opts) {
  ...
  self.initChildControls = function (prefix) {
    ...
    if (opts.canOrder) {
      $("#" + prefix + "-move-up").on("click", function () {
        ...
        var currentChildOrderElem = currentChild.children(
          'input[name$="-ORDER"]'
        );
  ...
  self.updateAddButtonState = function () {
    if (opts.maxForms) {
      var forms = $("> [data-inline-panel-child]", self.formsUl).not(
        ".deleted"
      );
  ...
  buildExpandingFormset(opts.formsetPrefix, {
    onAdd: function (formCount) {
      var newChildPrefix = opts.emptyChildFormPrefix.replace(
        /__prefix__/g,
        formCount
      );
...
$(function () {
  ...
  $previewButton.one("click", function () {
    if ($previewButton.data("auto-update")) {
      ...
      $form
        .on("change keyup DOMSubtreeModified", function () {
