...
.. note::
    ...
    You can register your own 'locale components' which allow you to add custom
    attributes to a locale, such as a flag or a currency. You can read more
    about this at `Extending locales with 'locale components'`_
...
Extending locales with 'locale components'
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

'locale components' allow you to add additional attributes to a locale such as
a flag, or a currency.

To create one, you need to define a model that has a ``OneToOneField`` to the
``Locale`` model. Then register it by decorating it with ``@register_locale_component``.
...
.. code-block:: python
    ...
    class FlagComponent(models.Model):
        locale = models.OneToOneField('wagtailcore.Locale', on_delete=models.CASCADE)
...
Once this model has been created and migrated, the "Flag" field will appear in
the locale management UI.

Customising the form
~~~~~~~~~~~~~~~~~~~~

Like pages*, you can define the form that is displayed for locale components
by specifying the ``panels`` attribute.

For example, we could replace the above "flag" field in the above example with
an image chooser:
...
.. code-block:: python
    ...
    class FlagComponent(models.Model):
        locale = models.OneToOneField('wagtailcore.Locale', on_delete=models.CASCADE)
        flag = models.ForeignKey('wagtailimages.image', on_delete=models.CASCADE)
...
.. note::
    ...
    \*Unlike pages, however, ``InlinePanel`` is not supported on locale components
