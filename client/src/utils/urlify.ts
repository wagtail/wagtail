/* eslint-disable no-prototype-builtins, no-restricted-syntax, id-length */
...
class Downcoder {
  ...
  constructor() {
    ...
    this.map = map;
    this.regex = new RegExp(chars.join('|'), 'g');
  }

  downcode(s: string) {
    return s.replace(this.regex, (m) => this.map[m]);
  }
}

const downcoder = new Downcoder();
...
function URLify(str: string, numChars?: number, allowUnicode = false) {
  ...
  if (allowUnicode) {
    ...
    s = s.replace(s, RegExp('[^-_\\p{L}\\p{N}\\s]', 'g'), '');
  } else {
    s = s.replace(/[^-\w\s]/g, ''); // remove unneeded chars
  }
  s = s.replace(/^\s+|\s+$/g, ''); // trim leading/trailing spaces
  s = s.replace(/[-\s]+/g, '-'); // convert spaces to hyphens
  s = s.substring(0, numChars); // trim to first num_chars chars
...
export const urlify = (value: string, options = {}) => {
  // URLify performs extra processing on the string and is more suitable
  // for creating a slug from the title, rather than sanitising a slug entered manually
  const cleaned = URLify(value, 255);
