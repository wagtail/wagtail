...
describe('TabsController', () => {
  ...
  const setup = async (
    ...
    <div id="test-tabs" data-controller="w-tabs" data-action="custom:event->w-tabs#select" data-w-tabs-active-class="animate-in">
      ...
      <button id="extra-trigger" type="button" data-action="w-tabs#select" data-w-tabs-target="trigger" data-w-tabs-id-param="tab-panel-2">Trigger for Tab 2</button>
    </div>
  `,
    definitions = [
  ...
  describe('tabs behavior', () => {
    ...
    it('should support non-tab triggers', async () => {
      await setup();
      expect(errors).toHaveLength(0);

      const [tab1, tab2] = document.querySelector('[role="tablist"]').children;
      const [tab1Panel, tab2Panel] =
        document.querySelector('.tab-content').children;
    ...
    it('should gracefully removing removing panel/trigger targets', async () => {
      await setup(`
      <div id="mutating-tabs" data-controller="w-tabs">
        <div role="tablist">
          <a id="tab-1" href="#tab-panel-1" role="tab" data-w-tabs-target="trigger" data-action="w-tabs#select:prevent">A</a>
  ...
  describe('handling transitions', () => {
    it('should allow for animated transition durations', async () => {
      await setup(`
    <div id="basic-tabs" data-controller="w-tabs" data-w-tabs-active-class="animate-in" data-w-tabs-transition-value="300">
      <div role="tablist">
        <a id="tab-1" href="#tab-panel-1" role="tab" data-w-tabs-target="trigger" data-action="w-tabs#select:prevent">A</a>
        <a id="tab-2" href="#tab-panel-2" role="tab" data-w-tabs-target="trigger" data-action="w-tabs#select:prevent">B</a>
  ...
  `);
      ...
      // run timers to 250ms first, check no change
      ...
      // run timers to 300ms, check transition class added
