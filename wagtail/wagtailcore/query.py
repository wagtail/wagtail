...
class PageQuerySet(MP_NodeQuerySet):
    ...
    def specific(self):
        """
        This efficiently gets a list of all specific pages in this queryset,
        using the minimum number of queries.

        As this returns a list, no further queries can be chained after a call
        to `specific`.
        """
...
def specific(qs):
    ...
    # Construct a list of all pages, in the order they occurred in the
    # original query.
    return [pages_by_type[content_type][pk]
            for pk, content_type in pks_and_types]
...
if DJANGO_VERSION >= (1, 9):
    ...
    class SpecificIterator(BaseIterator):
        def __iter__(self):
            return iter(specific(self.queryset))
