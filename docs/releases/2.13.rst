...
``Page.content_panels`` now includes `CommentPanel`, which is used to save and load comments. If you are overriding page edit handlers and
want to use the new commenting system, your list of edit handlers should be updated to include ``CommentPanel``.

New data attribute in rich text
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

To allow paragraphs in rich text to be identified consistently by the commenting system, the unique block key used in the Draftail editor
for each block level element is now saved in the generated database html in the ``data-block-key`` attribute.
