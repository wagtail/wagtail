...
### Removal of unused DOM `Range` polyfill

Any code that relied on the `window.rangy.getSelection()` for example will no longer function.

Either include the [Rangy polyfill](https://github.com/timdown/rangy) manually or migrate code to the official [`Document.createRange()`](https://developer.mozilla.org/en-US/docs/Web/API/Range) browser API.
