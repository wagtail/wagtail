...
### Private pages in PagesAPIViewSet

If the Pages endpoint for your Wagtail API v2 uses authentication, Wagtail will filter the output and return only pages accessible to the authenticated user. If your site has a large number of pages, this may incur a performance penalty. You may want to override `get_base_queryset` and `get_queryset` to control how many pages are checked for appropriate permissions. If your Pages endpoint does not use authentication, then `get_base_queryset` does not need to check permissions and returns a queryset containing all live, public pages.
