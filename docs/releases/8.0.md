...
Browser support for AVIF and WebP is now solid enough that Wagtail no longer converts images in these formats to PNG when no output format is specified. As a result, renditions of AVIF and WebP source images will now be served in their original format, which may change the file extension and content type of existing renditions.

If you need to retain the previous behavior, add the following to your settings:
...
See [](customizing_output_formats) for more details.
