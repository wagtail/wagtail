 ...
 * Replace jQuery autosize v3.0.4 with Stimulus `w-autosize` controller using autosize npm package v6.0.1 (Sudeep Ranjan Sahoo, Suyash Srivastava, LB (Ben) Johnston)
...
### Auto height/size text area widget now implemented with Stimulus
...
It is recommended that the `AdminAutoHeightTextInput` widget be used instead. You can also adopt the `data-controller` attribute and this will now function as before.

**Old syntax**

```html
<textarea id="story" name="story">It was a dark and stormy night...</textarea>
<script>autosize($('story'));</script>
```

**New syntax**

```html
<textarea id="story" name="story" data-controller="w-autosize">It was a dark and stormy night...</textarea>
```
