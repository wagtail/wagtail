...
class TestAuditLogManager(TestCase, WagtailTestUtils):
    ...
    def test_get_for_user(self):
        self.assertEqual(
            logging.PageLogEntry.objects.get_for_user(self.user).count(), 1
...
class TestAuditLog(TestCase):
    ...
    def test_page_edit(self):
        ...
        self.assertEqual(logging.PageLogEntry.objects.count(), 1)
        self.assertEqual(
            logging.PageLogEntry.objects.filter(action="wagtail.edit").count(), 1
        )

        # passing a string for the action should log this.
        self.home_page.save_revision(log_action="wagtail.revert")
        self.assertEqual(
            logging.PageLogEntry.objects.filter(action="wagtail.revert").count(), 1
        )
    ...
    def test_page_publish(self):
        ...
        self.assertEqual(logging.PageLogEntry.objects.count(), 1)
        self.assertEqual(
            logging.PageLogEntry.objects.filter(action="wagtail.publish").count(), 1
        )
    ...
    def test_page_rename(self):
        ...
        self.assertEqual(
            logging.PageLogEntry.objects.filter(action="wagtail.rename").count(), 0
        )
        ...
        self.assertEqual(
            logging.PageLogEntry.objects.filter(action="wagtail.publish").count(), 2
        )
        self.assertEqual(
            logging.PageLogEntry.objects.filter(action="wagtail.rename").count(), 1
        )
    ...
    def test_page_unpublish(self):
        ...
        self.assertEqual(logging.PageLogEntry.objects.count(), 1)
        self.assertEqual(
            logging.PageLogEntry.objects.filter(action="wagtail.unpublish").count(), 1
        )
    ...
    def test_revision_schedule_publish(self):
        ...
        log_entries = logging.PageLogEntry.objects.filter(
    ...
    def test_page_reorder(self):
        ...
        self.assertEqual(
            logging.PageLogEntry.objects.filter(
                action="wagtail.reorder", user=user
            ).count(),
            1,
        )
        self.assertEqual(
            logging.PageLogEntry.objects.filter(
                action="wagtail.move", user=user
            ).count(),
            0,
        )
    ...
    def test_page_delete(self):
        ...
        child.delete()
        self.assertEqual(
            logging.PageLogEntry.objects.filter(action="wagtail.delete").count(), 1
        )

        # check deleting a parent page logs child deletion
        self.home_page.delete()
        self.assertEqual(
            logging.PageLogEntry.objects.filter(action="wagtail.delete").count(), 3
        )
        self.assertListEqual(
            list(
                logging.PageLogEntry.objects.filter(
    ...
    def test_workflow_actions(self):
        ...
        workflows.WorkflowTask.objects.create(
            workflow=workflow, task=task_1, sort_order=1
        )
        workflows.WorkflowTask.objects.create(
            workflow=workflow, task=task_2, sort_order=2
        )
        ...
        workflow_entry = logging.PageLogEntry.objects.filter(
        ...
        for action in ["approve", "reject"]:
            with self.subTest(action):
                ...
                entry = logging.PageLogEntry.objects.filter(
    ...
    def test_workflow_completions_logs_publishing_user(self):
        ...
        self.assertFalse(
            logging.PageLogEntry.objects.filter(action="wagtail.publish").exists()
        )
        ...
        self.assertEqual(
            logging.PageLogEntry.objects.get(action="wagtail.publish").user, publisher
        )
    ...
    def test_page_privacy(self):
        ...
        self.assertEqual(
            logging.PageLogEntry.objects.filter(
                action="wagtail.view_restriction.edit"
            ).count(),
            1,
        )
...
class TestAuditLogHooks(TestCase, WagtailTestUtils):
    ...
    def test_action_must_be_registered(self):
        ...
        with self.assertRaises(ValidationError) as e:
            logging.PageLogEntry.objects.log_action(
                self.root_page, action="test.custom_action"
            )
    ...
    def test_action_format_message(self):
        ...
        log_entry = logging.PageLogEntry.objects.log_action(
