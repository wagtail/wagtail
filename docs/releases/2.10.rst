 ...
 * Remove markup around rich text rendering by default, provide a way to use old behaviour via ``wagtail.contrib.legacy.richtext``. See :doc:`legacy_richtext`. (Coen van der Kamp, Dan Braghis)
...
To restore the old behaviour, see :doc:`legacy_richtext`.
