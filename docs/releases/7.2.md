...
Due to the removal of the ad-hoc jQuery code to support async updates to the preview image & serving URL in the image URL generator, there is a rework of this area of code that may impact customizations.

The URL `wagtailimages:generate_url` and it's associated view class `GenerateURLView` for async JSON requests for image & serving URLs have been removed. The new approach uses the URL with name `wagtailimages:url_generator_output` and leverages the same `URLGeneratorView` as the page itself with async requests now returning HTML partials.

If you were overriding these URLs, please review the core code and assess how you can adapt your customizations to the new approach.

If you were overriding the template `wagtailimages/url_generator.html`, there are now major changes to this to leverage the partial HTML from a new inner template `wagtailimages/url_generator_output.html` instead.
