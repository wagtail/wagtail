...
class EditView(
    ...
    def action_name_and_method(self):
        if self.request.POST.get("action-publish") and self.page_perms.can_publish():
            ...
            self.updated_by_another_user = False
            # New Logic: Check if page has been updated by another user
            latest_revision = self.page.get_latest_revision()
            
            if (
                latest_revision
                and self.request.POST.get("page_revision_id")
                and int(latest_revision.id) != int(self.request.POST.get("page_revision_id"))
            ):
                self.updated_by_another_user = True
            
            # Return tuple of name and method
