...
class CopyForm(forms.Form):
    ...
    def clean(self):
        ...
        if slug and parent_page.get_children().filter(slug=slug).count():
            if hasattr(settings, 'WAGTAILADMIN_AUTO_INCREMENT_SLUG') and settings.WAGTAILADMIN_AUTO_INCREMENT_SLUG:
                cleaned_data['new_slug'] = self.page._get_autogenerated_slug(cleaned_data['new_slug'])
            else:
    ...
    def clean(self):
        ...
        if 'slug' in self.cleaned_data:
            if not Page._slug_is_available(cleaned_data['slug'], self.parent_page, self.instance):
                if hasattr(settings, 'WAGTAILADMIN_AUTO_INCREMENT_SLUG') and settings.WAGTAILADMIN_AUTO_INCREMENT_SLUG:
                    cleaned_data['slug'] = self.instance._get_autogenerated_slug(cleaned_data['slug'])
                else:
