...
export class BlockGroup {
  ...
  constructor(
    ...
    // Ensure all sub-groups have their collapsed label text set
    this.children.concat(this.settings).forEach((child) => {
      if (child instanceof BlockGroup) child.setCollapsedLabelText();
    });
  ...
  render(container: JQuery, prefix: string) {
    ...
    if (isRoot) {
      ...
      container.append(groupContainer ?? dom);
    }
    ...
    if (settings.length > 0) {
      ...
      const hidden = 'onbeforematch' in document ? 'until-found' : '';
      if (hasCustomTemplate) {
        blockSettings = dom.find('[data-structblock-settings]');
      ...
      // @ts-expect-error Normally, ActionButton subclasses expect a
      // BaseSequenceChild instance. However, SettingsButton doesn't need it,
      // as it can work with any collapsible panel.
      this.settingsButton = new SettingsButton({ element: dom.get(0) });
  ...
  renderChild(
    ...
    // it's a BlockGroupDefinition, render with a collapsible panel
