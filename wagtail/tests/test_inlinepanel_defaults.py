...
class MyComponent(models.Model):
    parent = ParentalKey("MySettings", on_delete=models.CASCADE, related_name="mycomponent", unique=True)
...
class MySettings(BaseSiteSetting, ClusterableModel):
    panels = [InlinePanel("mycomponent", min_num=1, max_num=1)]
...
class TestInlinePanelDefaults:
    def test_inline_with_defaults_saves_without_changes(self):
        ...
        root_page, _ = Page.objects.get_or_create(title="Root", slug="root", depth=1, path="0001", locale=locale)
        site = Site.objects.filter(is_default_site=True).first()
        if not site:
            site = Site.objects.create(hostname="localhost", root_page=root_page, is_default_site=True)

        settings = MySettings.for_site(site)
        ...
        assert form.is_valid(), f"Form should be valid but got: {form.errors.as_text()}"
