...
class FilteredSelect(forms.Select):
    ...
    they are picked up by the Stimulus `RulesController` (w-rules).
    Relies on the root form having the suitable data attributes set.
    See examples and tests in client/src/controllers/RulesController.ts for more details
    """
    ...
    def create_option(
        ...
        if filter_value is not None:
            ...
            rules = {self.filter_field: ["", *[str(val) for val in filter_value]]}
            option["attrs"] = {
                **(option["attrs"] or {}),
                "data-w-rules": json.dumps(rules),
                "data-w-rules-target": "enable show",
            }
