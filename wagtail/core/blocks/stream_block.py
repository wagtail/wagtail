...
class BaseStreamBlock(Block):
    ...
    def render_list_member(
        self, block_type_name, value, prefix, index, errors=None, id=None
    ):
        """
        ...
        return render_to_string(
            "wagtailadmin/block_forms/stream_member.html",
            {
                    ...
                    mark_safe(
                        escape_script(
                            self.render_list_member(
                                name, child_block.get_default(), "__PREFIX__", ""
                            )
                        )
                    ),
                )
    ...
    def render_form(self, value, prefix="", errors=None):
        ...
        if errors:
            if len(errors) > 1:
                ...
                # blocks
                raise TypeError(
                    "StreamBlock.render_form unexpectedly received multiple errors"
                )
        ...
        valid_children = [
            child for child in value if child.block_type in self.child_blocks
        ]
        ...
        return render_to_string(
            "wagtailadmin/block_forms/stream.html",
            {
                "prefix": prefix,
