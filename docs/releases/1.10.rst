...
methods is any custom template tag your project may include for generating navigation menus.
...
and passing ``*args, **kwargs`` at the point where you are calling ``get_url_parts`` on ``super`` (if applicable), e.g.:

.. code-block:: python

	super(MyPageModel, self).get_url_parts(*args, **kwargs)

While one could pass only the ``request`` keyword argument, passing all arguments as-is ensures compatibility with any
future changes to these method signatures.

See: :meth:`wagtail.wagtailcore.models.Page.get_url_parts`, :meth:`wagtail.wagtailcore.models.Page.get_url`,
