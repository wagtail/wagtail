 ...
 * Removed default responsive embed padding and ``responsive-object`` class around embeds (Kalob Taulien)
 * Added ``WAGTAILEMBEDS_RESPONSIVE_HTML`` setting to enable automatic addition of ``responsive-object`` around embeds (Kalob Taulien)
