...
The v3 API is released as a preview to signify we want to adapt it based on feedback: it may change in backwards-incompatible ways in any release until stabilised. We welcome feedback via discussions, feature requests and bug reports on [our project repository](https://github.com/wagtail/wagtail). This feature was developed by Thibaud Colas and Sage Abdullah.
...
### Change to internal `FilteredSelect` cross-field referencing

If you are using `wagtail.admin.widgets.FilteredSelect`, the `filter_field` kwarg is no longer the HTML `id` of the related field, it should be changed to be the HTML `name` of the related field instead.

for most usage, this will simply mean removal of the `id_` prefix on the provided value, this will depend on your customization though.
...
### JQuery backed `filtered-select` widget has been removed

The undocumened internal selector for `data-widget="filtered-select"` which would attach behavior to support filtering of options in a related select input has been removed. This has been replaced with a new Stimulus controller, `RulesController`, this new approach is not yet officially documented.

However, similar behaviour can be achieved by replacing the following attributes.

| Attribute location        | Old                                                                  | New                                                                         | Details                                                                                               |
| ------------------------- | -------------------------------------------------------------------- | --------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------- |
| Container element         | (no special attribute)                                               | `data-controller="w-rules"` with `data-action="change->w-rules#resolve"`    | This will be the `form` element or `fieldset` if the behavior is contained within a subset of fields. |
| Filtered `select` element | `data-widget="filtered-select"` + `data-filter-field="id_continent"` | `data-w-rules-target="show"` with `data-w-rules='{"continent": ["value"]}'` |                                                                                                       |
| `option` elements         | `data-filter-value="..."`                                            | Not applicable; show/hide entire select instead                             |                                                                                                       |
