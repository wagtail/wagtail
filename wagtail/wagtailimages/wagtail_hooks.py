...
from wagtail.wagtailimages.admin_modules import ImageModelModule


@hooks.register('register_admin_urls')
def register_admin_urls():
    image_module = ImageModelModule('wagtailimages', model=get_image_model())

    return [
        url(r'^images/', include(image_module.urls)),
    ]
...
def register_images_menu_item():
    return ImagesMenuItem(_('Images'), urlresolvers.reverse('wagtailimages:index'), classnames='icon icon-image', order=300)
