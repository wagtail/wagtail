...
.. Note::
    ...
    The CSS classes added to images do *not* come with and accompanying stylesheet, or inline styles. e.g the ``left`` class will do nothing, by default. The developer is expected to add these classes to their front end CSS files, to define what exactly ``left``, ``right`` or ``full-width`` means *to them*.
