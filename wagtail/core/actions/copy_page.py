...
def copy_page(page, recursive=False, to=None, update_attrs=None, copy_revisions=True, keep_live=True, user=None,
    ...
    for (child_relation, old_pk), child_object in child_object_map.items():
        if process_child_object:
            process_child_object(specific_page, page_copy, child_relation, child_object)
    ...
    if _mpnode_attrs:
        ...
        if to:
            if recursive and (to == page or to.is_descendant_of(page)):
