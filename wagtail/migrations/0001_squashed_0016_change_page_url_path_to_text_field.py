...
def initial_data(apps, schema_editor):
    ...
    # Create default site
    Site.objects.using(db).get_or_create(
        hostname="localhost", root_page_id=homepage.id, is_default_site=True
    )

    # Create auth groups
    moderators_group, created = Group.objects.using(db).get_or_create(name="Moderators")
    editors_group, created = Group.objects.using(db).get_or_create(name="Editors")
