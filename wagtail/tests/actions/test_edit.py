...
class TestEditAction(WagtailTestUtils, TestCase):
    ...
    def test_edit_live_draftstate_model_does_not_update_live_object(self):
        # Create a live (published) DraftStateMixin instance.
        instance = DraftStateModel.objects.create(text="Original", live=True)
        instance.text = "Draft edit"
        EditAction(instance, user=self.user).execute()

        # The live DB record must not be overwritten, only the revision gets the
        # new text.
        instance.refresh_from_db()
