...
export class UnsavedController extends Controller<HTMLFormElement> {
   ...
   */
  get formData() {
    const exclude = ['overwrite_revision_id', 'csrfmiddlewaretoken', 'next'];
  ...
  checkIntervalValueChanged(newInterval: number, oldInterval?: number) {
    if (this.forceValue) {
    ...
    const oldNotifyDuration =
      (oldInterval || 0) + UnsavedController.notifyDelayValue;
