...
### Swappable page view restrictions and custom privacy options

Page view restrictions are now fully extensible, allowing developers to define custom access control logic—such as paywalls or subscription tiers—while completely reusing the core Wagtail privacy UI. 

This change introduces two new configuration settings:
* `WAGTAIL_PAGE_VIEW_RESTRICTION_MODEL` allows you to swap out the default restriction model with a custom model that subclasses `AbstractPageViewRestriction`.
* `WAGTAIL_PAGE_PRIVACY_OPTIONS` allows you to customize or append the list of restriction options surfaced to administrators in the page privacy dialog.

Existing configurations remain fully backward compatible. For details on implementation, see [](custom_page_view_restriction).

This feature was developed by Yousuf Abu Shanab.
