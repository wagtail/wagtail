...
Wagtail uses most of Tailwind’s core plugins, with an override for them to create [Logical properties and values](https://rtlstyling.com/posts/rtl-styling#css-logical-properties) styles (using `tailwindcss-vanilla-rtl`) while still using Tailwind’s default utility and design token names.

#### Class naming and prefixing

Every Tailwind utility class generated by Wagtail is prefixed with `w-` to prevent collisions with other stylesheets (e.g., `w-flex`, `w-bg-surface-page`, `w-p-4`). 

Tailwind variants (e.g., `sm:`, `hover:`, `dark:`, `focus:`) are placed *before* the prefix:
* `sm:w-flex`
* `hover:w-bg-secondary-hover`
* `dark:w-bg-grey-700`
...
* **Border Radius**:
  ...
  * `w-rounded` (`0.3125rem` / 5px, default)
  * `w-rounded-md` (`0.625rem` / 10px)
  * `w-rounded-xl` (`1.5rem` / 24px)
  * `w-rounded-full` (`100%`)
* **Border Width**:
  * `w-border` (`0.0625rem` / 1px, default)
  * `w-border-0` (`0`)
  * `w-border-2` (`0.125rem` / 2px)
  * `w-border-5` (`0.3125rem` / 5px)
* **Box Shadow**:
  * `w-shadow` (`5px 5px 20px rgba(0,0,0,0.05)`, default)
  * `w-shadow-md` (`5px 5px 30px var(--w-color-box-shadow-md)`)
...
2. **Semantic Theme Colors** (defined in [client/src/tokens/colorThemes.js](https://github.com/wagtail/wagtail/blob/main/client/src/tokens/colorThemes.js)):
   ...
   * **Borders**: `border-furniture`, `border-button-small-outline-default`, `border-field-default`, `border-field-inactive`, `border-field-hover`, `border-button-outline-default`, `border-button-outline-hover`, `border-interactive-more-contrast`, `border-furniture-more-contrast`.
   * **Icons**: `icon-primary`, `icon-primary-hover`, `icon-secondary`, `icon-secondary-hover`.
   * **Misc**: `focus`, `box-shadow-md`.
   * *Example utilities*: `w-bg-surface-page`, `w-text-text-meta`, `w-border-border-furniture`.
...
* **Avoid utilities relating to font size, weight, or other typographic considerations.** Instead, use the higher-level type scale component classes defined in [client/src/tokens/typography.js](https://github.com/wagtail/wagtail/blob/main/client/src/tokens/typography.js):
  ...
  * *Example usage*: `<h2 class="w-h2">Page Title</h2>`

##### Custom variants
In addition to standard Tailwind variants, Wagtail provides custom variants:
* `expanded:` – targets components with `aria-expanded="true"` (e.g., `expanded:w-block`).
* `more-contrast:` – targets users with a high-contrast theme active or preferred.
* `forced-colors:` – targets Windows High Contrast mode via the `@media (forced-colors: active)` media query (e.g., `forced-colors:w-border-ButtonText`).
