...
class Block(metaclass=BaseBlock):
    ...
    def all_media(self):
        ...
        # container blocks (e.g. a RichTextBlock appearing at the top level of a StreamField
        # as well as both sides of a StructBlock for producing two-column layouts), we will
        # encounter identical media declarations. Adding these to the final combined media
        # declaration would be redundant add processing time when determining the final media
        # ordering. To avoid this, we keep a cache of previously-seen declarations and only
        # add unique ones.
        media_cache = set()
