...
export class TabsController extends Controller {
  ...
  /** Track whether the controller has connected. */
  ready?: boolean;
  ...
  select(
    ...
    const { focus = false, id = '' } = {
      id: (target && target.getAttribute('aria-controls')) || '',
   ...
   */
  syncLocation() {
    if (!this.ready) return null; // do not run if the controller has not connected yet
    if (!this.useLocationValue) return null;
