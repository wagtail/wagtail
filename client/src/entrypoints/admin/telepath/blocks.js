...
class ListBlock {
  ...
  setError(errorList) {
    ...
    // eslint-disable-next-line no-restricted-syntax
    for (const blockId in error.blockErrors) {
      if (error.blockErrors.hasOwnProperty(blockId)) {
        this.childBlocks[blockId].setError(error.blockErrors[blockId]);
      }
    }
  }
...
class StreamBlock {
  ...
  setError(errorList) {
    ...
    for (const blockId in error.blockErrors) {
      if (error.blockErrors.hasOwnProperty(blockId)) {
        this.children[blockId].setError(error.blockErrors[blockId]);
      }
    }
  }
