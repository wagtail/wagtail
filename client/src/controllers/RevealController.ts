...
export class RevealController extends Controller<HTMLElement> {
   ...
   */
  toggle() {
    const newCLosedValue = !this.closedValue;

    this.stored = this.peekingValue ? false : newCLosedValue;
  ...
  /**
   * Set the stored state (if closed) of this controller in the local storage.
