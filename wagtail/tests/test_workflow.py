...
class TestWorkflows(TestCase):
    ...
    def test_add_task_to_workflow(self):
        ...
        workflows.WorkflowTask.objects.create(
            workflow=workflow, task=task, sort_order=1
        )
        self.assertIn(
            task, workflows.Task.objects.filter(workflow_tasks__workflow=workflow)
        )
        self.assertIn(
            workflow, workflows.Workflow.objects.filter(workflow_tasks__task=task)
        )
    ...
    def test_get_specific_task(self):
        ...
        group_approval_task = workflows.GroupApprovalTask.objects.create(
    ...
    def test_publishing_page_cancels_workflow_when_cancel_on_publish_true(self):
        ...
        self.assertEqual(
            workflow_state.status, workflows.WorkflowState.STATUS_CANCELLED
        )
    ...
    def test_publishing_page_does_not_cancel_workflow_when_cancel_on_publish_false(
        ...
        workflow_state.refresh_from_db()
        self.assertEqual(
            workflow_state.status, workflows.WorkflowState.STATUS_IN_PROGRESS
        )
    ...
    def cancel_workflow(self):
        ...
        self.assertEqual(
            workflow_state.current_task_state.status,
            workflows.TaskState.STATUS_CANCELLED,
        )
        self.assertFalse(
            workflows.TaskState.objects.filter(
                workflow_state=workflow_state,
                status=workflows.TaskState.STATUS_IN_PROGRESS,
            ).exists()
        )
    ...
    def test_task_workflows(self):
        workflow = workflows.Workflow.objects.create(name="test_workflow")
        disabled_workflow = workflows.Workflow.objects.create(
        ...
        workflows.WorkflowTask.objects.create(
            workflow=disabled_workflow, task=task, sort_order=1
        )
    ...
    def test_tasks_with_state(self):
        ...
        self.assertEqual(
            tasks[0].task_state.status, workflows.TaskState.STATUS_IN_PROGRESS
        )
        ...
        tasks = workflow_state.all_tasks_with_state()
        self.assertEqual(
            tasks[0].task_state.status, workflows.TaskState.STATUS_APPROVED
        )
        self.assertEqual(
            tasks[1].task_state.status, workflows.TaskState.STATUS_REJECTED
        )
        ...
        self.assertEqual(
            tasks[1].task_state.status, workflows.TaskState.STATUS_IN_PROGRESS
        )
        self.assertEqual(
            tasks[1].task_state,
            workflows.TaskState.objects.filter(workflow_state=workflow_state).order_by(
                "-started_at", "-id"
            )[0],
        )
