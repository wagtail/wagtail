...
.. class:: LinkHandler
    ...
    .. method:: get_identifier()
        ...
        Required. The static ``get_identifier`` method is expected to return a string that can be used to identify which rich text tags should be handled by this handler.
    ...
    .. method:: get_model()
        ...
        For example, ``PageLinkHandler.expand_db_attributes`` returns the Wagtail class ``Page``.
...
Below is an example custom rewrite handler that implements these methods to add support for rich text linking to user email addresses. It supports the conversion of rich text tags like ``<a linktype="user" username="wagtail">`` to valid HTML like ``<a href="mailto:hello@wagtail.io">``. This example assumes that equivalent frontend functionality has been added to allow users to insert these kinds of links into their rich text editor.
...
.. method:: FeatureRegistry.register_link_type_handler(handler)
...
.. code-block:: python
    ...
    def register_link_handler(features):
        features.register_link_type_handler(MyCustomLinkHandler)

.. method:: FeatureRegistry.register_embed_type_handler(handler)
...
.. code-block:: python
    ...
    def register_embed_handler(features):
        features.register_embed_type_handler(MyCustomEmbedHandler)

Two other methods exist that allow you to register simple functions that implement custom handlers as bare functions that implement only ``expand_db_attributes`` as described above. These methods will be removed in a future version of Wagtail.
