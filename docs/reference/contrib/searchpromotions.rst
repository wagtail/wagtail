...
.. module:: wagtail.contrib.searchpromotions
...
The ``searchpromotions`` module is not enabled by default. To install it, add ``wagtail.contrib.searchpromotions`` to ``INSTALLED_APPS`` in your project's Django settings file.
