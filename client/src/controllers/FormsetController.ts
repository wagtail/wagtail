 ...
 */
const afterTransition = (element: HTMLElement, { maxDelay = 350 } = {}) => {
  /**
...
export class FormsetController extends Controller<HTMLElement> {
  ...
  childTargetDisconnected(target: HTMLElement) {
    ...
    )(target.classList.add(...deletedClasses)).then(() => {
  ...
  get newChild() {
    ...
    // ensure all scripts from the new template source will run
    // by creating new script elements and replacing the old ones
    newChildNode.querySelectorAll('script').forEach((script) => {
