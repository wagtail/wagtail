...
class TestFormsSubmissionsList(WagtailTestUtils, TestCase):
    def setUp(self):
        # Create a form page
        self.form_page = make_form_page()
        self.url = reverse('wagtailforms:list_submissions', args=[self.form_page.pk])
    ...
    def test_export_urls_include_filters(self):
        ...
        # Send a request with a filter parameter
        response = self.client.get(self.url, data={'foo': 'baz'})

        # Check that the URLs generated by the view include the appropriate filtering parameters
        view = response.context['view']
        expected_csv_url = reverse('wagtailforms:export_submissions', args=[self.form_page.pk]) + '?foo=baz'
        expected_xlsx_url = reverse('wagtailforms:export_submissions', args=[self.form_page.pk]) + '?foo=baz&type=xlsx'
        self.assertEqual(view.csv_export_url, expected_csv_url)
        self.assertEqual(view.xlsx_export_url, expected_xlsx_url)
