...
describe('encodeForm utility', () => {
  ...
  it('handles file inputs by using the filename', () => {
    const form = document.createElement('form');
    const file = new File(['dummy content'], 'document.pdf', {
      type: 'application/pdf',
    });

    // Mock FormData.entries to include a File object
    const originalFormData = window.FormData;
    window.FormData = class extends originalFormData {
      entries() {
        return [['file', file]][Symbol.iterator]();
      }
    };
    ...
    try {
      const result = encodeForm(form);
      const params = new URLSearchParams(result);
      expect(params.get('file')).toBe('document.pdf');
    } finally {
