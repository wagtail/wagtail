...
class TestEditorHtmlImageEmbedHandler(TestCase, WagtailTestUtils):
    def test_get_db_attributes(self):
        ...
        self.assertEqual(
            EditorHtmlImageEmbedHandler.get_db_attributes(soup.b),
            {
                'id': 'test-id',
                'alt': 'test-alt',
                'format': 'test-format',
    ...
    def test_expand_db_attributes_for_editor(self):
        ...
        self.assertTagInHTML(
            (
                '<img '
                'data-embedtype="image" data-id="1" data-format="left" '
...
class TestFrontendImageEmbedHandler(TestCase, WagtailTestUtils):
    def test_expand_db_attributes_for_frontend(self):
        ...
        self.assertTagInHTML(
            '<img class="richtext-image left" alt="test-alt" />',
            result,
            allow_extra_attrs=True
        )
    ...
    def test_expand_db_attributes_for_frontend_with_nonexistent_image(self):
        self.assertEqual(
            FrontendImageEmbedHandler.expand_db_attributes({'id': 0}),
            '<img>'
        )
