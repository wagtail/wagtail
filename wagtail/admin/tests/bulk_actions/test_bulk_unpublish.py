...
class TestBulkUnpublish(TestCase, WagtailTestUtils):
    def setUp(self):
        ...
        self.url = reverse('wagtailadmin_bulk_unpublish', args=(self.root_page.id, )) + '?'
    ...
    def test_unpublish_view_invalid_page_id(self):
        ...
        response = self.client.get(reverse('wagtailadmin_bulk_unpublish', args=(12345, )))
    ...
    def test_after_unpublish_page(self):
        ...
        def hook_func(request, page):
            ...
            self.assertEqual(page.id, self.pages_to_be_unpublished[self.i].id)
            self.i += 1
    ...
    def test_before_unpublish_page(self):
        ...
        def hook_func(request, page):
            self.assertIsInstance(request, HttpRequest)
            self.assertEqual(page.id, self.pages_to_be_unpublished[self.i].id)
            self.assertEqual(page.status_string, _("live"))
            self.i += 1
