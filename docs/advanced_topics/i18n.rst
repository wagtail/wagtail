...
This document descibes how to configure Wagtail for authoring content in
multiple languages and, optionally, set up a workflow that allows users to
translate content between languages.

This document only covers internationalisation of content managed in Wagtail.
...
For example, if you have two sites in two locales, then you will see four page
homepages at the top level of the page hierarchy in the explorer.

Translations are linked together in the database so that translations
to be continuously kept up to date with their original (see `Translation Workflow`_)
and language selectors in the UI can work. But as they are in separate trees, the pages can
be structured differently and even have fully translated URL paths!
...
When creating a site in Wagtail, you have to select a single page as the site's
"root page", which would become the homepage of the site. Multi-lingual sites
have multiple homepages, so how do we make it aware of those?

As mentioned above, page translations are linked in the database. Wagtail uses
this feature to find the homepage of a translated site. So if the user's
language is different to the site's root page, it'll look for a translation of
it instead. If a translation is not found, or is unpublished, it will use the
version that was assigned in the "root page" field as the default.

This means that to translate a site into another locale, you just need to
translate and publish its homepage in that new locale. You can also use the
"root page" field to specify a different default fallback for your sites if you
want to.
...
stores the BCP-47 language code that represents this locale.
...
by the ``WAGTAIL_CONTENT_LANGUAGES`` setting.
 ...
 .. note:: Read this if you've changed ``LANGUAGE_CODE`` before enabling internationalisation
    ...
    On initial migration, Wagtail createsa a ``Locale`` record for the language that
    ...
    you will need to manually update record in the ``Locale`` model too before
...
Here is a basic example of how to add linkes between translations of a page.
...
otherwise skip to the next section that has more complicated example that takes
this into account.
...
All page instnces in Wagtail have a ``.localized`` attribute which fetches the translation
...
Create the data migration with the following command:
...
.. code-block:: python
    ...
    class Migration(migrations.Migration):
        ...
        # Add one operation for each model to boostrap here
...
implementation as they use slightly different approches, so there's a chance that
onr of them might fit your use case better:
