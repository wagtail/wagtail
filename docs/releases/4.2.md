...
### `window.LockUnlockAction` is no longer available

The undocumented JavaScript global `LockUnlockAction` will no longer be available, this is now implemented with Stimulus and relies on data attributes only.
